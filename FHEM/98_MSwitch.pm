...
sub MSwitch_Set_SetTrigger($@) {
	...
	#MSwitch_LOG( $name, 0,"args @args".__LINE__);
	#MSwitch_LOG( $name, 0,"-----------------------------------".__LINE__);
...
#MSwitch_LOG( $Name, 0,"@testidsdev" );
#MSwitch_LOG( $Name, 0,"---- ");
 # MSwitch_LOG( $Name, 0,"@testidsdev1" );
#   MSwitch_LOG( $Name, 0,"\n" );   
...
sub MSwitch_Getsupport($) {
	...
	#MSwitch_LOG( $Name, 0,"dets $Name tcond -> $tmp" );
...
$out .= "keine Deviceactions definiert " if @affecteddevices < 1;
 #MSwitch_LOG( $Name, 0,"affecteddevices -@affecteddevices-" );
 ...
 #$out = "";
    ...
    asyncOutput( $hash->{CL},
"<html><center>Bei Supportanfragen bitte untenstehene Datei anhängen.<br>&nbsp;<br><textarea name=\"edit1\" id=\"edit1\" rows=\""
          . "200\" cols=\"200\" STYLE=\"font-family:Arial;font-size:9pt;\">"
          . $out
          . "</textarea><br></html>" );
    return;
}
...
sub MSwitch_Sysextension($) {
   ...
   my $out = MSwitch_Asc(ReadingsVal( $Name, '.sysconf', '' ));
	...
	# my $out = ReadingsVal( $Name,'.sysconf', 'undef' );
